...
def get_product_by_id(db:Session , product_id:int,current_user_id:int):
  return db.query(Product).filter(Product.id==product_id,Product.is_delete!=True,Product.owner_id==current_user_id).first()

def get_products(db:Session ,current_user_id:int):
  return db.query(Product).filter(Product.is_delete!=True,Product.owner_id==current_user_id).all()
...
def update_product(db:Session , product_id:int , data:ProductUpdate,current_user_id:int):
  product_to_update=get_product_by_id(db, product_id,current_user_id)
...
def soft_delete_products(db:Session , product_id:int,current_user_id:int):
  product_to_delete = get_product_by_id(db, product_id,current_user_id)
